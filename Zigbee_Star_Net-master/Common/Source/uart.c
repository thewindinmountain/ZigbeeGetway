 ...
 *
 *\COMPONENT           $HeadURL: https://www.collabnet.nxp.com/svn/lprf_apps/Application_Notes/JN-AN-1069-IEEE-802.15.4-Serial-Cable-Replacement/Trunk/Common/Source/uart.c $
 *
...
#if (USR_UART_SEL_DIO15)
#define USR_UART_CFG FALSE
#else
#define USR_UART_CFG TRUE
#endif
/****************************************************************************/
/***        Macro Definitions                                             ***/
/****************************************************************************/
		...
		QUEUE_tsData   *psRxQueue)		/**< Receive queue pointer */
{
	...
	if (u8Uart < 2)
	{
	    ...
	    /* Select UART IO */
	    if(E_AHI_UART_1 == u8Uart)
	    {
	    	vAHI_UartSetLocation(u8Uart, USR_UART_CFG);//TRUE for DIO9, FALSE for DIO15
	    	if(USR_UART_CFG == TRUE)
	    	{
	    		DBG_vPrintf(UART_TRACE, "\r\n usrUART point to DIO9\r\n ");
	    	}else
	    	{
	    		DBG_vPrintf(UART_TRACE, "\r\n usrUART point to DIO15\r\n ");
	    	}
	    }
	    /* Enable UART */
...
PRIVATE void UART_vTxDataToFifo(uint8 u8Uart)
{
    uint8 u8Data;
