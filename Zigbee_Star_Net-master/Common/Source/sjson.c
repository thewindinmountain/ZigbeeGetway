...
char *SJSON_ObjToNext(char *obj, int *type, char **end)
{
	...
	value = SJSON_ObjToValue(obj, type, end); /* get type and end */
	if (!value) return 0; /* malformed */
...
char *SJSON_ToIdxObj(char *value, unsigned idx, int *type, char **end)
{
	...
	value = SJSON_ObjToValue(obj, type, end); /* get type and end in case of idx 0*/
	if (!value) return 0; /* malformed */

	while (idx>0)
	{
		idx--;
		obj = SJSON_ObjToNext(obj, type, end);
...
char *SJSON_ToIdxObjValue(char *value, unsigned idx, int *type, char **end)
{
	...
	obj = SJSON_ToIdxObj(value, idx, 0, 0);
	if (!obj) return 0; /* not found */

	return SJSON_ObjToValue(obj, type, end);
}
...
char *SJSON_ToObj(char *value, const char *name, int *type, char **end)
{
	...
	obj = SJSON_ToIdxObj(value, 0, 0, 0);
	if (!obj) return 0; /* malformed */

	while (obj)
	{
		if (!SJSON_ObjStrcmp(obj, name)) 
		{
			value = SJSON_ObjToValue(obj, type, end); /* get type and end */
			if (!value) return 0; /* malformed */ 
			return obj; 
...
char *SJSON_ToObjValue(char *value, const char *name, int *type, char **end)
{
	...
	obj = SJSON_ToObj(value, name, 0, 0);
	if (!obj) return 0; /* not found */
...
char *SJSON_ToPathValue(char *value, const char *path, int *type, char **end)
{
	...
	{	
		...
		{
			out = SJSON_ToPathValue(SJSON_ObjToValue(obj, 0, 0), path, type, end);
			if (out) return out;
			obj = SJSON_ObjToNext(obj, 0, 0);
		}
	...
	{
		path = &path[i+1];
		value = SJSON_ToObjValue(value, name, 0, 0);
		if (!value) return 0; // not found
		return SJSON_ToPathValue(value, path, type, end);
	}
	...
	if (path[i] == '[')
	{
		...
		if (*name != '[') value = SJSON_ToObjValue(value, name, 0, 0);
		if (!value) return 0; // not found
		value = SJSON_ToArrayValue(value, n, type, end);
	...
	// path ends
	if (!path[i]) return SJSON_ToObjValue(value, name, type, end);
...
char *SJSON_FindObjValue(char *value, const char *name, int *type, char **end)
{
	...
	value = SJSON_ToObjValue(value, name, type, end);
	if (value) return value; /* object value found */
	
	while (obj) 
	{
		value = SJSON_ObjToValue(obj, 0, 0);
		value = SJSON_FindObjValue(value, name, type, end);
		if (value) return value; /* object value found */
