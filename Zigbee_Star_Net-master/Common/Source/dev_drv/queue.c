 ...
 *\MODULE              Queue
 *
 *\COMPONENT           $HeadURL: https://www.collabnet.nxp.com/svn/lprf_apps/Application_Notes/JN-AN-1069-IEEE-802.15.4-Serial-Cable-Replacement/Trunk/Common/Source/queue.c $
 *
...
/****************************************************************************/
/**
 * <b>QUEUE_bGet</b> &mdash; get a byte from a serial queue and not remove it
 */
/****************************************************************************/
PUBLIC bool_t QUEUE_bGetWithOffset(QUEUE_tsData *psQueue, 	/**< Queue to operate on. */
		                            uint16 		offset,     /**< offset for reads byte */
				       	 	 	    uint8  	 	*pu8Item)	/**< Location for byte */
{
	uint16 u16Head;
	bool_t bReturn = FALSE;
	...
	if (psQueue != (QUEUE_tsData *) NULL && pu8Item != (uint8 *) NULL)
	{
		/* return false if we have not enough data */
		if(QUEUE_u16Used(psQueue) < offset)
			return FALSE;
		...
		if (psQueue->u16Tail != u16Head)
		{
			...
			if (psQueue->u16Tail > u16Head)
			{
				*pu8Item = psQueue->pu8Data[psQueue->u16Tail+offset];;
			}
			else
			{
				if((psQueue->u16Mask + 1 - psQueue->u16Tail) > offset)
				{
					*pu8Item = psQueue->pu8Data[psQueue->u16Tail + offset];
				}
				else
				{
					*pu8Item = psQueue->pu8Data[offset - (psQueue->u16Mask + 1 - psQueue->u16Tail)];
				}
			}
			bReturn = TRUE;
		}
